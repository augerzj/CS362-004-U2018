...
int testSmithy(struct gameState *G, int smithyPosition);
...
int main() {
	...
	printf("----------RANDOM CARD TEST #1 - TEST OF SMITHY----------\n");
	...
	for (tries = 0; tries < 1000000; tries++) {
		...
		// set position of smithy in hand
		smithyPosition = rand() % G.handCount[G.whoseTurn];
		G.hand[G.whoseTurn][smithyPosition] = smithy;

		currTestResult = testSmithy(&G, smithyPosition);
		if(currTestResult != 1) {
			testPass = 0;
		}
	}
	...
	if(testPass == 0) {
		printf("SMITHY DID NOT PASS ALL TESTS\n\n\n");
	}
	else {
...
int testSmithy(struct gameState *G_point, int smithyPosition) {
	int testPass = 1;
	int bonus = rand();
	...
	cardEffect(smithy, rand(), rand(), rand(), &G, smithyPosition, &bonus);
